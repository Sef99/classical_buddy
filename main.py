...
async def process_composer(ctx, composer_details, wikipedia_desc):
...
async def _help(ctx):
    ...
    embed_message.add_field(name='!rec composer', value='Recommends and plays a random piece from the given composer.', inline=False)
...
async def _search_name(ctx, composer_name):
    ...
    wikipedia_desc = wikiapi(composer_name)
    await process_composer(ctx, composer_details, wikipedia_desc)
